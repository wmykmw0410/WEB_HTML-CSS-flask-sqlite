...
各章は「機能の学習」と「`000_my_app`を完成させるための機能追加」の2部構成です。前者は`example/`で単体のサンプルとして学び、後者は`challenge/`に取り組みます（`100_bookstore_api`を完成形の参考にしながら、`000_my_app`に組み込むルーティングの練習です）。
...
### 問題：002_html_cssのブックストアをFlaskで配信し、書籍詳細ページを追加しよう

`002_html_css/challenge/answer`で作った静的なブックストアのトップページを土台に、Flaskで配信できるようにし、さらに書籍詳細ページを追加します。`challenge/challenge.py`を開き、TODOコメントの指示に従って以下の仕様を満たすルーティングを実装してください。テンプレート（`challenge/templates/top.html`・`challenge/templates/detail.html`）とCSS・画像（`challenge/static/`）は完成済みなので、Python側のルーティングだけを実装します。ここで作るルーティングは、この先の章で`000_my_app`に組み込んでいきます（完成形は`100_bookstore_api`を参考にします）。
...
| エンドポイント | メソッド | 処理 |
|---|---|---|
| `/` | GET | `top.html`を描画する（002_html_cssと同じ書籍一覧ページ。変数は不要） |
| `/books/<int:book_id>` | GET | `detail.html`を描画する（動的ルーティングで受け取った`book_id`から該当書籍を探して表示する） |
| 上記で`book_id`に該当する書籍がある場合 | — | `title`・`author_line`（`"著者: xxx"`）・`price_line`（`"¥xxx"`）・`image`（`/static/img/xxx.png`）を渡す |
| 上記で`book_id`に該当する書籍が無い場合 | — | `title`に`"書籍ID {book_id} は見つかりません"`、`author_line`・`price_line`に空文字、`image`に`/static/img/not_found.png`を渡す |

`top.html`の書籍一覧の各カードは、すでに`/books/1`〜`/books/5`へのリンクになっています。

#### ヒント

- `books`は`{1: {"title": ..., "author": ..., "price": ..., "image": ...}, ...}`のような辞書。`books.get(book_id)`で該当データを取得する（無ければ`None`）
- 「見つかった場合／見つからなかった場合」の表示の出し分けは、テンプレート側の`{% if %}`ではなく、Python側の`if`文で`title`・`author_line`などの中身を作り分けることで実現する（`{% if %}`のようなJinja2の条件分岐は`006_jinja2`で学ぶ）
- 画像・CSSは`static/`フォルダに置くと、Flaskが自動で`/static/ファイル名`というURLで配信してくれる（`url_for('static', filename=...)`という書き方は`006_jinja2`で学ぶので、ここでは`/static/...`と直接書けばよい）
