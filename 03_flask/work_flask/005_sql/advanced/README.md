...
└── answer/
    ├── 01.sql   問題 1 解答（IN）
    ├── 02.sql   問題 2 解答（BETWEEN）
    ├── 03.sql   問題 3 解答（IS NULL）
    ├── 04.sql   問題 4 解答（ALTER TABLE）
    ├── 05.sql   問題 5 解答（COMMIT）
    └── 06.sql   問題 6 解答（ROLLBACK）
```
...
> [answer/01.sql](answer/01.sql) 〜 [answer/06.sql](answer/06.sql) — 解答
...
| 1 | 特定カテゴリの商品を取得 | IN |
| 2 | 価格帯で絞り込み | BETWEEN |
| 3 | 在庫未登録の商品を取得 | IS NULL |
| 4 | カラムを追加 | ALTER TABLE |
| 5 | 複数件をまとめて追加 | BEGIN / COMMIT |
| 6 | 削除を取り消す | BEGIN / ROLLBACK |
