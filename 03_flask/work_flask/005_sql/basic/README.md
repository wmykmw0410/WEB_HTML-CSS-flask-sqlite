...
│   └── practice.sql    練習問題
└── answer/
    ├── 01_create.sql   問題 1 解答
    ├── 02_insert.sql   問題 2 解答
    ├── 03_select.sql   問題 3 解答
    ├── 04_update.sql   問題 4 解答
    ├── 05_delete.sql   問題 5 解答
    └── 06_join.sql     JOIN 練習問題 解答
```
...
> [example/practice.sql](example/practice.sql) — 問題
> [answer/01_create.sql](answer/01_create.sql) 〜 [answer/06_join.sql](answer/06_join.sql) — 解答

### 問題：メンバー管理テーブルを操作しよう

`practice.sql` を開き、コメントの指示に従って SQL を完成させてください。
...
| 1 | `members` テーブルを作成する |
| 2 | 5件のデータを INSERT する |
| 3 | SELECT で一覧・部署絞り込み・salary 降順・平均・DISTINCT・GROUP BY・HAVING を取得する |
| 4 | 特定メンバーの salary を UPDATE する |
| 5 | 条件に一致するメンバーを DELETE する |

JOIN の練習は `example/06_join.sql` と `answer/06_join.sql` を参照してください。
