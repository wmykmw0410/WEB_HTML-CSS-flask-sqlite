...
このチャプターを終えると `100_memo_api` のログイン部分（`auth/`）が読めるようになります。
...
```bash
cd 015_login/example
python 01_flask_wtf.py
```

ブラウザで `http://127.0.0.1:5041/` にアクセスします。

| 確認する操作 | 確認したいこと |
|---|---|
| ユーザー名・パスワードを両方空欄のまま送信する | 画面遷移せず、「ユーザー名を入力してください」「パスワードを入力してください」という赤字のエラーが両方のフィールドの下に表示される |
| ユーザー名に51文字以上の文字列を入力して送信する | 「50 文字以内で入力してください」というエラーが表示される |
| ユーザー名・パスワードを正しく入力して送信する | 緑字で「送信完了：ユーザー名 = （入力した値）」と表示される |

**正常な状態の見分け方**：エラーがある間は緑の送信完了メッセージが出ず、赤字のエラーだけが表示された状態で同じ入力画面にとどまります。すべて正しく入力したときだけ緑のメッセージに切り替わることを確認してください。
...
### 動作確認：ログイン済み/未ログインで見える画面の違い

| 確認する操作 | 確認したいこと |
|---|---|
| ログインしていない状態で`http://127.0.0.1:5042/mypage`に直接アクセスする | マイページは表示されず、`/login`にリダイレクトされる（`@login_required`が未ログインを検知し、`login_manager.login_view`に設定した`'login'`に飛ばす） |
| `/register`で新しいユーザー名・パスワードを登録する | 登録後、flashメッセージ「登録が完了しました。ログインしてください。」とともに`/login`に遷移する |
| 登録したユーザー名・パスワードで`/login`する | `/mypage`にリダイレクトされ、「ようこそ、（ユーザー名） さん」「ログイン中の user.id：（数値）」と表示される |
| 存在しないユーザー名、または間違ったパスワードでログインを試みる | `/login`のまま「ユーザー名またはパスワードが正しくありません。」と表示され、`/mypage`には進まない |
| ログイン後にトップページ（`/`）へアクセスする | 「ログイン中：（ユーザー名）」と表示され、マイページへのリンクが出る（未ログイン時は「ログイン / 新規登録」リンクだった箇所が変わる） |
| `/logout`にアクセスした後、再び`/mypage`にアクセスする | 再び`/login`にリダイレクトされる（ログアウトでセッションが破棄され、`@login_required`が未ログイン扱いに戻る） |

**正常な状態の見分け方**：`@login_required`が付いたルート（`/mypage`）は、ログインしていない間は必ず`/login`に弾かれ、ログインしている間だけ本来の内容が表示されます。ログアウト直後に`/mypage`へ戻ってもう一度弾かれることまで確認できると、セッションが正しく破棄されているとわかります。
...
### 動作確認：追加した4つの機能がそれぞれ効いているか

`http://127.0.0.1:5043`で動作確認します。

| 問題 | 確認する操作 | 確認したいこと |
|---|---|---|
| 1 | 登録フォームで`password`と`confirm`に**違う値**を入力して送信する | 登録されずに「パスワードが一致しません」のようなエラーが表示される |
| 2 | 正しいユーザー名・パスワードでログインする | ログイン後の画面にフラッシュメッセージ「ログインしました」が表示される |
| 3 | ログイン済みの状態で`/login`に直接アクセスする | ログインフォームは表示されず、トップページにリダイレクトされる |
| 4 | すでに登録済みのユーザー名でもう一度登録しようとする | 登録されずにエラーメッセージが表示される（同じユーザー名が2件登録されない） |

**正常な状態の見分け方**：4つの問題はそれぞれ独立した機能追加なので、1つずつ個別に確認できます。どれか1つでも「本来弾かれるべき操作が通ってしまう」場合は、対応するバリデーター・条件分岐の実装漏れを疑ってください。

---

## 5. 練習問題：メモデータの管理にログイン機能を組み込もう

> [challenge/challenge.py](challenge/challenge.py) — 問題 ｜ [challenge/answer/challenge.py](challenge/answer/challenge.py) — 解答

### 問題：ログインしたユーザーだけがメモを追加できるようにしよう

`014_flask_migrate`で作ったメモ一覧・詳細・メモ追加フォーム・リダイレクト（`challenge/challenge.py`にすでに実装済み）に、Flask-Login を組み込みます。新規登録・ログイン・ログアウトの機能を実装し、`/memos/new`をログイン必須にします。

`Migrate(app, db)`はすでに設定済みなので、`User`モデルを定義したら`flask db migrate`でマイグレーションファイルを生成し、`memos`テーブルに加えて`users`テーブルを追加してください。
...
| 4 | `/memos/new`に`@login_required`を追加する |
...
- 見た目やCSRFの仕組みは`014_flask_migrate`から変更不要

#### 動作確認：ログインしていないとメモを追加できないか
...
| 確認する操作 | 確認したいこと |
|---|---|
| ログインしていない状態で`http://127.0.0.1:5045/memos/new`に直接アクセスする | メモ追加フォームは表示されず、flashメッセージ「ログインしてください。」（`login_manager.login_message`）とともに`/login`にリダイレクトされる |
| `/register`で新規登録し、`/login`でログインする | ログイン後、メモ一覧（`/`）にリダイレクトされる |
| ログインした状態で`/memos/new`にアクセスする | 今度はメモ追加フォームが表示される |
| フォームに入力してメモを追加する | メモ一覧に新しいメモが追加されて表示される |
| `/logout`でログアウトしてから再度`/memos/new`にアクセスする | またフォームが表示されず、`/login`に押し戻される |

**正常な状態の見分け方**：`/memos/new`はログインしている間だけ表示され、ログアウトすると即座に`/login`へ弾かれる、という状態が保たれているかを確認してください。
