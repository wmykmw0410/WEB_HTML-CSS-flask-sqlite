...
app.config['SQLALCHEMY_DATABASE_URI']        = 'sqlite:///' + os.path.join(base_dir, 'answer04.sqlite')
...
with app.app_context():
    memo = Memo.query.filter_by(id=1).first()
    print(f"問題4: id=1 の title = {memo.title}")

# クリーンアップ
db_path = os.path.join(base_dir, 'answer04.sqlite')
